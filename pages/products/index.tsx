...
const sortOptions = [
  ...
  { name: 'Highest', href: '#', current: false },
  { name: 'Lowest', href: '#', current: false },
]
const subCategories = [
  { name: 'Totes', href: '#' },
  { name: 'Backpacks', href: '#' },
...
const ProductsList: FC<FCTypes> = ({ products }) => {
  ...
  const handleSortOptionSelected = (selectOption: TProductListSort, event: any) => {
    event.preventDefault();
    setSort(selectOption);
    const sortedProducts = sortProducts(filteredProducts, selectOption);
  ...
  const handleFilterChange = (event: any) => {
    ...
    const selectedCategories = updatedFilters[0].options
                        ...
                        <Menu.Item key={option.name}>
                          {({ active }) => (
                            <a
                              ...
                              onClick={(event) => handleSortOptionSelected(option.name as TProductListSort, event)}
                            >
                              {option.name}
                            </a>
                          )}
